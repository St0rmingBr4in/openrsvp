<script lang="ts">
	...
	async function handleSubmit() {
		...
		try {
			...
			toast.success($_('feedbackModal.submittedThank') + (allowFollowUp ? $_('feedbackModal.confirmationSent') : ''));
			message = '';
			feedbackType = 'bug';
			allowFollowUp = true;
			open = false;
		} catch {
			toast.error($_('feedbackModal.submitFailed'));
		} finally {
			submitting = false;
		}
	}
...
<button
	...
	aria-label={$_('feedbackModal.sendFeedback')}
>
...
<Modal bind:open title={$_('feedbackModal.title')}>
	<form onsubmit={(e) => { e.preventDefault(); handleSubmit(); }}>
		<div class="space-y-4">
			<div>
				<label for="feedback-type" class="block text-sm font-medium text-neutral-700 mb-1">{$_('feedbackModal.type')}</label>
				<select
					id="feedback-type"
				...
				>
					<option value="bug">{$_('feedbackModal.typeBug')}</option>
					<option value="feature">{$_('feedbackModal.typeFeature')}</option>
					<option value="general">{$_('feedbackModal.typeGeneral')}</option>
				</select>
			</div>

			<div>
				<label for="feedback-message" class="block text-sm font-medium text-neutral-700 mb-1">{$_('feedbackModal.message')}</label>
				<textarea
					id="feedback-message"
					bind:value={message}
					rows="5"
					maxlength="2000"
					required
					placeholder={$_('feedbackModal.messagePlaceholder')}
			...
			<label class="flex items-start gap-3 cursor-pointer">
				...
				/>
				<span class="text-sm text-neutral-600">{$_('feedbackModal.followUp')}</span>
			</label>
		</div>
		...
		<div class="mt-4 flex justify-end gap-3">
			...
			>
				{submitting ? $_('feedbackModal.submitting') : $_('feedbackModal.submit')}
			</button>
		</div>
	</form>
</Modal>

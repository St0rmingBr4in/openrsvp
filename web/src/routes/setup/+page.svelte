<script lang="ts">
	...
	onMount(async () => {
		...
		try {
			...
			if (apiErr.status === 401 || apiErr.status === 403) {
				...
				toast.error($_('setup.loadFailed'));
			}
		} finally {
			loading = false;
		}
	...
	function validate(): boolean {
		...
		if (supportEmail.trim() && !EMAIL_RE.test(supportEmail.trim())) {
			errors.supportEmail = $_('setup.invalidEmail');
		}
	...
	async function handleSubmit() {
		...
		try {
			...
			configured = true;
			toast.success($_('setup.configured'));
		} catch (e: unknown) {
			const apiErr = e as { status?: number; message?: string };
			if (apiErr.status === 401 || apiErr.status === 403) {
				needsAdmin = true;
				toast.error($_('setup.adminRequiredSave'));
			} else {
				toast.error(apiErr.message || $_('setup.saveFailed'));
			}
		} finally {
			submitting = false;
		}
		...
		{:else if needsAdmin}
			<Card>
				<div class="text-center py-6 space-y-4">
					<h1 class="text-xl font-bold font-display text-neutral-900">{$_('setup.adminSigninHeading')}</h1>
					<p class="text-sm text-neutral-500">
						{$_('setup.adminSigninBody')}
					</p>
					<div class="pt-2">
		...
		{:else}
			<div class="mb-8">
				<h1 class="text-2xl font-bold font-display text-neutral-900">
					{configured ? $_('setup.headingSettings') : $_('setup.headingWelcome')}
				</h1>
			...
			<Card>
				<div class="space-y-6">
					<Input
						label={$_('setup.instanceName')}
						name="instanceName"
						bind:value={instanceName}
						placeholder={$_('setup.instanceNamePlaceholder')}
					...
					<fieldset class="pt-1">
						<legend class="text-sm font-medium text-neutral-700 mb-2">{$_('setup.organizerSignups')}</legend>
						<label class="flex items-start gap-3 cursor-pointer">
							...
							/>
							<div>
								<span class="text-sm text-neutral-700">{$_('setup.allowSignups')}</span>
								<p class="text-xs text-neutral-400">
									{$_('setup.allowSignupsHelp')}
								</p>
							</div>
						</label>
					</fieldset>
					...
					<Input
						label={$_('setup.supportEmail')}
						name="supportEmail"
						type="email"
						bind:value={supportEmail}
						placeholder={$_('setup.supportEmailPlaceholder')}

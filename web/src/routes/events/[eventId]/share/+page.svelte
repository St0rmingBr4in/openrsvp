<script lang="ts">
	...
	onMount(async () => {
		...
		if (eventResult.data) {
			...
			toast.error(apiErr.message || $_('share.loadFailed'));
		} finally {
			loading = false;
		}
	});
	...
	async function copyLink() {
		try {
			...
			copied = true;
			toast.success($_('share.linkCopied'));
			setTimeout(() => (copied = false), 2000);
		} catch {
			toast.error($_('share.copyFailed'));
		}
	}
</script>

<svelte:head>
	<title>{$_('share.pageTitle')}</title>
</svelte:head>

<AppShell>
	<div class="max-w-3xl mx-auto">
		<div class="mb-6">
			<a href="/events/{eventId}" class="text-sm text-primary hover:text-primary-hover">{$_('share.backToEvent')}</a>
			<h1 class="mt-2 text-2xl font-bold font-display text-neutral-900">{$_('share.heading')}</h1>
			{#if event}
		...
		{:else if event}
			...
			<Card class="mb-6">
				{#snippet header()}
					<h2 class="text-lg font-semibold font-display text-neutral-900">{$_('share.shareLink')}</h2>
				{/snippet}

				<p class="text-sm text-neutral-600 mb-4">
					{$_('share.shareLinkDesc')}
				</p>
				...
				<div class="flex items-center gap-2">
					...
					<Button onclick={copyLink} variant={copied ? 'secondary' : 'primary'} size="md">
						{copied ? $_('share.copied') : $_('share.copyLink')}
					</Button>
				</div>
			</Card>
			...
			<Card class="mb-6">
				{#snippet header()}
					<h2 class="text-lg font-semibold font-display text-neutral-900">{$_('share.qrCode')}</h2>
				{/snippet}

				<div class="flex flex-col items-center py-6">
					{#if qrDataUrl}
						<img src={qrDataUrl} alt={$_('share.qrAlt')} class="w-48 h-48 rounded-lg" />
					{:else}
			...
			<Card>
				{#snippet header()}
					<h2 class="text-lg font-semibold font-display text-neutral-900">{$_('share.responseSummary')}</h2>
				{/snippet}

				<div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
					<div class="text-center">
						<p class="text-2xl font-bold font-mono text-success">{stats.attending}</p>
						<p class="text-xs text-neutral-500">{$_('share.statAttending')}</p>
					</div>
					<div class="text-center">
						<p class="text-2xl font-bold font-mono text-warning">{stats.maybe}</p>
						<p class="text-xs text-neutral-500">{$_('share.statMaybe')}</p>
					</div>
					<div class="text-center">
						<p class="text-2xl font-bold font-mono text-error">{stats.declined}</p>
						<p class="text-xs text-neutral-500">{$_('share.statDeclined')}</p>
					</div>
					<div class="text-center">
						<p class="text-2xl font-bold font-mono text-info">{stats.pending}</p>
						<p class="text-xs text-neutral-500">{$_('share.statPending')}</p>
					</div>
				</div>
				<div class="mt-4 pt-4 border-t border-neutral-200 text-center">
					<p class="text-sm text-neutral-600">
						<span class="font-semibold">{stats.total}</span> {$_('share.totalInvitees')}
					</p>
				</div>
			</Card>
		{/if}

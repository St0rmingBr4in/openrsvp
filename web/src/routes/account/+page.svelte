<script lang="ts">
	...
	async function handleExport() {
		...
		try {
			...
			toast.error(apiErr.message || $_('account.exportFailed'));
		} finally {
			exporting = false;
		}
	}
	...
	async function handleDelete() {
		...
		try {
			...
			const apiErr = err as { message?: string };
			toast.error(apiErr.message || $_('account.deleteFailed'));
		} finally {
			deleting = false;
		}
	}
...
{:else if $currentUser}
		...
		<div class="max-w-3xl mx-auto">
			...
			<Card>
				<div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
					<div class="sm:pr-8">
						<h2 class="text-lg font-display font-semibold text-neutral-900">{$_('account.yourData')}</h2>
						<p class="mt-1 text-sm text-neutral-600">
							{$_('account.yourDataDesc')}
						</p>
					</div>
			...
			<Card class="mt-6 border-error-light">
				<div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
					<div class="sm:pr-8">
						<h2 class="text-lg font-display font-semibold text-error">{$_('account.dangerZone')}</h2>
						<p class="mt-1 text-sm text-neutral-600">
							{$_('account.dangerDesc')}
						</p>
					</div>
					<div class="flex-shrink-0">
	...
	<Modal bind:open={deleteModalOpen} title={$_('account.deleteModalTitle')}>
		<div class="space-y-4">
			<div class="rounded-lg bg-error-light border border-error px-4 py-3 text-sm text-error flex items-start gap-2">
				...
				<span>
					{$_('account.deleteWarnPre')}<strong>{$_('account.deleteWarnBold')}</strong>{$_('account.deleteWarnPost')}
				</span>
			</div>
			<p class="text-sm text-neutral-600">
				{$_('account.confirmPre')}<span class="font-mono font-semibold text-neutral-900">{$_('account.confirmWord')}</span>{$_('account.confirmPost')}
			</p>
			<Input
				name="deleteConfirm"

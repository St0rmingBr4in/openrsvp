...
<div class="min-h-screen flex items-center justify-center px-4">
	<div class="w-full max-w-md">
		<div class="text-center mb-8">
			...
			<p class="mt-2 text-neutral-600">{$_('auth.login.subtitle')}</p>
		</div>
			...
			{#if sent}
				...
				<div class="text-center">
					...
					<p class="text-sm text-neutral-600 mb-4">
						{$_('auth.login.checkEmailPre')}<strong>{email}</strong>{$_('auth.login.checkEmailPost')}
					</p>
					<p class="text-xs text-neutral-500 mb-6">
						{$_('auth.login.checkSpam')}
					</p>
					<Button
						variant="outline"
						onclick={() => {
			...
			{:else}
				...
				<form onsubmit={handleSubmit} class="space-y-6">
					...
					<Button type="submit" {loading} class="w-full">
						{loading ? $_('auth.login.sending') : $_('auth.login.sendButton')}
					</Button>
				</form>

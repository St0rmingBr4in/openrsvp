<script lang="ts">
	...
	onMount(async () => {
		...
		if (!t) {
			view = 'invalid';
			errorMessage = $_('unsubscribe.invalidLink');
			return;
		}
		token = t;
		...
		try {
			...
			if (apiErr.status === 404 || apiErr.status === 400) {
				errorMessage = $_('unsubscribe.invalidExpired');
			} else {
				errorMessage = apiErr.message || $_('unsubscribe.loadFailed');
			}
		}
	});
	...
	async function handleConfirm() {
		...
		try {
			...
			const apiErr = err as ApiError;
			view = 'error';
			errorMessage = apiErr.message || $_('unsubscribe.completeFailed');
		} finally {
			submitting = false;
		}
	}
		...
		<div class="bg-surface rounded-xl shadow-lg border border-neutral-200 p-8 text-center">
			{#if view === 'loading'}
				<div class="flex flex-col items-center gap-4 py-4">
					...
					<p class="text-neutral-500 text-sm">{$_('unsubscribe.checking')}</p>
				</div>
			{:else if view === 'invalid'}
				...
				<h1 class="font-display text-xl font-semibold text-neutral-900 mb-2">{$_('unsubscribe.invalidHeading')}</h1>
			...
			{:else if view === 'confirm'}
				...
				<h1 class="font-display text-xl font-semibold text-neutral-900 mb-2">{$_('unsubscribe.confirmHeading')}</h1>
				<p class="text-neutral-600 text-sm mb-1">
					{$_('unsubscribe.confirmPre')}<span class="font-medium text-neutral-900">{email}</span>{$_('unsubscribe.confirmMid')}{scopeLabel}{$_('unsubscribe.confirmSuffix')}
				</p>
				<p class="text-neutral-400 text-xs mb-6">{$_('unsubscribe.noFurther')}</p>
			...
			{:else if view === 'success'}
				...
				<h1 class="font-display text-xl font-semibold text-neutral-900 mb-2">{$_('unsubscribe.successHeading')}</h1>
				<p class="text-neutral-600 text-sm">
					{#if email}
						<span class="font-medium text-neutral-900">{email}</span>{$_('unsubscribe.successMid')}{scopeLabel}{$_('unsubscribe.successSuffix')}
					{:else}
						{$_('unsubscribe.successNoEmail')}
					{/if}
				</p>
				<p class="text-neutral-400 text-xs mt-4">
					{$_('unsubscribe.reSubscribe')}
				</p>
			{:else if view === 'error'}
				<div class="w-16 h-16 rounded-full bg-error-light flex items-center justify-center mx-auto mb-4">
					<svg class="w-8 h-8 text-error" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
				...
				<h1 class="font-display text-xl font-semibold text-neutral-900 mb-2">{$_('unsubscribe.errorHeading')}</h1>
